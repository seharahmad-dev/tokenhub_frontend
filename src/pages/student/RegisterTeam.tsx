// frontend: src/pages/student/RegisterTeam.tsx
...
export default function RegisterTeam() {
  ...
  // participants to send in backend: include leader + selected teammates
  ...
  async function handleCreatePayment() {
    ...
    try {
      // call your payment API to create an order
      // amount should be determined by event; for demo we'll use 100 (replace with actual event fee)
      const amount = 100; // replace with dynamic event fee when available
  ...
  async function handleFinalizeRegistration() {
    ...
    try {
      ...
      // optionally redirect to /student/events or show confirmation
    } catch (err: any) {
          ...
          <SectionCard title="Register a Team">
            <div className="space-y-4">
              <div>
                ...
                <input value={teamName} onChange={(e) => setTeamName(e.target.value)} className="mt-2 w-full rounded-lg border px-3 py-2" />
              </div>

              <div>
                <label className="block text-sm font-medium">Leader</label>
                <div className="mt-2 rounded-lg border bg-white px-3 py-2">
                  <div className="text-sm font-medium">{leaderName}</div>
              ...
              <div className="mt-4 flex gap-2">
                <button disabled={loadingPayment} onClick={handleCreatePayment} className="rounded bg-blue-600 px-3 py-2 text-white">
                  {loadingPayment ? "Creating payment…" : "Create Payment Order"}
                </button>
                <button onClick={() => { setSelected([]); setTeamName(""); }} className="rounded border px-3 py-2">
                  Reset
                </button>
              </div>
                ...
                <div className="rounded border bg-white p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <button onClick={handleCopyOrder} className="rounded bg-emerald-600 px-3 py-2 text-white text-sm">
                  ...
                  <div className="mt-3 text-sm text-slate-600">
                    After creating the order, complete the payment using your Razorpay flow. When you receive the final payment id (Razorpay payment id) paste it below and click Finalize Registration.
                  </div>

                  <div className="mt-3">
                    <label className="block text-sm">Paste payment id</label>
                    <input value={pastePaymentId} onChange={(e) => setPastePaymentId(e.target.value)} className="mt-2 w-full rounded-lg border px-3 py-2" />
                    <div className="mt-2 flex gap-2">
                      <button onClick={handleFinalizeRegistration} disabled={creating} className="rounded bg-blue-600 px-3 py-2 text-white">
